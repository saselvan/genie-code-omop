...
| `patient_identifier` | `PatientID` + identity type | `person.person_source_value` | Join for MRN and other identifiers. |
| `encounter` | `EncounterID` | `visit_occurrence`, `observation_period` (rollup) | Encounter window drives visits. |
| `encounter_diagnosis` | `EncounterID` + diagnosis rows | `condition_occurrence` | Diagnosis codes → Condition domain. |
| `procedure_dim` | `ProcedureID` (reference) | supporting dim for `procedure_occurrence` | Procedure dictionary / descriptions. |
| `procedure_order` | `OrderID` | `procedure_occurrence` | Orders with procedure codes & times. |
| `medication_order` | `OrderID` | `drug_exposure` | Medication orders; NDC / formularies. |
| `clinical_measurement` | `MeasID` | `measurement` (vitals, flowsheets) | Numeric/text results → LOINC or local codes. |
...
## `patient_identifier` — column hints
...
## `encounter_diagnosis` — column hints
...
## `clinical_measurement` — column hints
