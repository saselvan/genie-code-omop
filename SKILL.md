...
metadata:
  ...
  v2_0_5_notes: "AD-001 documentation rollout + spec correctness pass + harness diagnostics. First post-v2.0.4 release cycle, executed as three parallel work streams. Stream 1 (AD-001 docs, PE-001/002/003/004/013/018/020): the architectural decision that the skill validates 20 tables and auto-builds 14 — with the 6 v2.0.4a tables as validation-only via the BYO-ETL pattern — graduated from BACKLOG to canonical narrative documents (SKILL.md, scripts/_omop_dag.py docstring, references/omop_dag_dependencies.md). The build-vs-validate split is now first-class documentation rather than implicit lore. Seven Lens 1 PE findings closed alongside (phase-reference scrub completion, configs/person.yaml stale-reference cleanup with category-aware redirects, pipeline_generic.yml ↔ jobs.yml comment-template synchronization). Stream 2 (spec correctness, DC-001/002/003/004/007/010/011): six OHDSI v5.4 fidelity findings closed, plus one borderline-case decisions log seeded (references/spec_domain_decisions.md). Customer-visible behavior change in DC-003: drug_exposure.drug_exposure_end_date is now correctly enforced as Required=Yes per OHDSI v5.4 — customers with NULL values in this column will see new Layer 5 findings counting affected rows; the validator does not modify or reject data, only its findings surface the previously silent-passed case. DC-011 introduces 'Metadata' as a Domain assignment for the first time anywhere in the spec (note.encoding_concept_id; concept 32678 'UTF-8' resolves to Metadata domain per OHDSI WebAPI). Stream 3 (rehearsal harness diagnostics): the harness now captures pre-inject + post-cleanup schema state per call; a session-level capture_full_catalog_baseline helper formalizes the manual baseline capture pattern from v2.0.4c Commit 3, mechanically validating AD-001 against the live customer catalog (14 buildable tables captured / 6 BYO-ETL tables missing as expected). 506 → 522 test baseline. v2.0.5 also restructured BACKLOG.md from surface-of-origin grouping to release-bucket grouping (v2.0.5 scope / Deferred to v2.0.6 / Deferred to v2.1+) and recorded four cycle process-discipline lessons in SESSION-STATE: AskQuestion no-response defaults to surface-and-pause not best-judgment-decide; audit grep methodology should disambiguate placeholders and iterate beyond first-match; the same path can have multiple use-cases requiring different fix shapes (canonical-example vs customer-generated); verification grep should run before drafting stale-reference fixes, not as a post-draft gate. See SESSION-STATE.md for full post-mortem and BACKLOG.md for v2.0.6 candidates."
  v2_0_6_notes: "AD-001 customer-rollout completion + validator output bridges to documentation. Second post-v2.0.4 release cycle, executed as three parallel work streams (bundled-prompt cycle pattern; v2.0.6-scope.md drove triage of the 44 Lens 7 + Lens 3 findings into 11 in-scope items + 9 hitchhikers, ~9 days engineering / ~2 weeks). Stream 1 (AD-001 customer-coherence, TC-001/002/003/004/005/021/022/023 + UX-001/008/018/019/020): AD-001 narrative landed on 4 customer-facing surfaces — scaffolded README's new 'Validation scope vs build scope' section (TC-001), runbook new Section 7.5 'BYO-ETL: validation-only tables' (TC-002, single-page customer prose with three loading patterns for device_exposure / note+note_nlp / specimen-visit_detail-dose_era), recommended_ci_config.md differential validation subsection (TC-003), and new customer-facing templates/project_scaffold/docs/CHANGELOG.md (UX-008, v2.0.5 entry seeded). Validator's table_missing message gained AD-001 framing pointing to the just-shipped runbook Section 7.5 (UX-001), preserving 'does not exist in catalog', 'Subsequent layers will skip', and 'FAIL: table' backward-grep anchors. Stream 2 (validator output bridges, UX-002/004/006): FAIL trailers added to OMOP fidelity validator and Pydantic schema validator pointing to runbook Section 8 troubleshooting. PASS Summary enrichment surfaces per-layer counts (columns matched, PK status, concept_id columns checked, domain mismatches, NOT NULL violations) plus total rows checked and (on FAIL) layer-named failure framing — customer-visible behavior change isolated per the v2.0.5 stream 2 Commit 3 / DC-003 precedent. LayerResult.total_rows added as an additive dataclass field; notebook validator surface unchanged (consumes findings only, ignores total_rows). Standard-resolved % omitted as a v2.0.7 candidate (would require Layer 3 standard_concept query, which is DC-005's epic territory). Stream 3 (vocabulary glossary + carryovers, TC-007 + PE-008 + h8 + v2.0.5 carryovers): silver/core glossary on contributor (bundle_state.py docstring) and customer (omop-runbook.md Appendix D) surfaces — closes TC-007 and PE-008 via the cheaper glossary path; the v2.0.2 silver/core rename batch carryover is closed-by-supersession. BuildScopeError replaces three KeyError sites in _omop_dag.py with typed AD-001-framed exception (v2.0.5 carryover; scope expanded from 1 site to 3 during pre-flight per user judgment call). New contract test asserts pipeline_generic.yml ↔ jobs.yml enumeration lockstep, catching the drift class PE-020 surfaced. h9 hitchhiker corrects omop_dag_dependencies.md:74 placeholder count (10 → 13). 522 → 542 test baseline (+4 Stream 3 + 14 Stream 2 + 2 Stream 1 = +20 net new tests). Three cycle process-discipline observations recorded in SESSION-STATE: pre-flight verification catches scope drift consistently (Stream 2's wrong file location for Summary, Stream 3's KeyError site count, Stream 1's aspirational target_tables flag — each caught before drafting prose or code); bundled stream prompts work for tight cycles (one prompt per stream covering all commits with explicit per-commit gates, suitable for cycles where commits are mechanically simpler than v2.0.5's spec-correctness work); customer-facing prose deliverables need verification gates like code does (cross-reference verification on Section 7.5 / Section 8 / AD-001-in-BACKLOG / CHANGELOG citations, same shape as code's correctness gates). v2.0.6 also completes the AD-001 architectural rollout begun in v2.0.5 stream 1: contributor surfaces in v2.0.5, customer surfaces (README + runbook + CI doc + CHANGELOG + validator messages + BuildScopeError) in v2.0.6. The build-side BuildScopeError and validation-side table_missing message together bracket AD-001 across both customer-visible failure modes. See SESSION-STATE.md for full post-mortem and BACKLOG.md for v2.0.7 candidates."
---
